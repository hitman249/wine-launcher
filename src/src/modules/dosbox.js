...
export default class Dosbox {
  ...
  runConfig(config) {
    return this.update.downloadDosbox().then(() => {
      ...
      return `${dosbox} -conf ${conf} -c "mount C \\"${driveC}\\"" -c "C:" -c "cd ${gameDir}" -c "${gameExe}" -c exit ${config.isWindow() ? '' : '-fullscreen'}`;
    });
  }
  ...
  runFile(path, args= '') {
    return this.update.downloadDosbox().then(() => {
      ...
      return `${dosbox} -conf ${conf} -c "mount C \\"${driveC}\\"" -c "C:" -c "cd ${gameDir}" -c "${gameExe} ${args}" -c exit`;
    });
  }
